document.addEventListener("readystatechange", async(state) => {
    ...
    if (location.pathname == "/" && readyState == "complete") {
...
async function channelActions(onion) {
    try {
        ...
        if (Math.random() < onion.actions.clickVideoFromChannel.chance) {
            ...
            tabs[1].click();
            await delay(Math.floor(Math.random() * 350) + 3500 * 0.95)

            const videos = await getNode(() => document.querySelectorAll('ytd-two-column-browse-results-renderer #dismissible'), 15, () => Math.floor(Math.random() * 150) + 600 * 0.95);

            const video = videos[Math.floor(Math.random() * (videos.length - 1))];

            const video_a = await getNode(() => video.querySelector('a[href*="/watch"]'), 10, () => Math.floor(Math.random() * 150) + 500 * 0.95)
            video_a.click()
    ...
    const video = await getNode(() => document.querySelector("video"), 10, () => Math.floor(Math.random() * 150) + 550 * 0.95);
    // if (video) {
    console.log(Math.min((video.duration * 1000), 180000) || 0);
    chrome.runtime.sendMessage({
        msg: "closeTab",
        data: {
            options: {},
            delay: Math.min((video.duration * 1000), 180000) || 0
        }
    });
    // }
    chrome.runtime.sendMessage({
...
async function locationCheck(
    ...
    try {
        ...
        chrome.runtime.sendMessage({
            msg: "pressSearchEnter"
        });

        ///////////////////////////

        // const input = document.querySelector('input#search')
        // const query = 'YEAT TYPE BEAT'
...
async function load(URLS, index, name, onion, options) {
    ...
    if (document.location.pathname == "/results") {
        await search(URLS, index, onion, name, options);
        ...
        view(URLS, index, onion, name, options);
    }
}
async function search(URLS, index, onion, name, options) {
...
async function view(URLS, index, onion, name, options) {
    try {
        let time = 0;
        const jno = onion.keys;
        ...
        let actions = {
            ...
            clickChannel: {
                chance: Math.random() < actionsJson.clickChannel.chance,
        ...
        if (actions.clickChannel.chance) {
            actions.clickDescriptionLink.chance = false;
        }
            ...
            try {
                ...
                if (((!options.develop ? spent_time >= watchUpTo && currentTime >= watchUpTo : currentTime >= watchUpTo) && !isSomethingDoingRN(actions)) || spent_time > 180) {
                    ...
                    try {
                        ...
                        await delay(Math.floor(Math.random() * 150) + 20000 * 0.95)
                    } catch (error) {
                        console.error(error);
                        chrome.runtime.sendMessage({
                            msg: "sendErrorReport",
                    ...
                    try {
                        ...
                        let metadata = await getNode(() => document.querySelector('ytd-watch-metadata'), 10, () => Math.floor(Math.random() * 150) + 350 * 0.95)
                        let segmentLikeBtn = await getNode(() => metadata.querySelector('#segmented-like-button'), 10, () => Math.floor(Math.random() * 150) + 350 * 0.95)

                        let likeBtn = await getNode(() => segmentLikeBtn.querySelector('button'), 10, () => Math.floor(Math.random() * 150) + 350 * 0.95)
                    ...
                    const descriptionNode = await getNode(() => document.querySelector('#description-inner'), 10, () => Math.floor(Math.random() * 150) + 350 * 0.95)
                    ...
                    if (descriptionNode) {
                        ...
                        const links = await getNode(() => descriptionNode.querySelectorAll('a[target="_blank"]'), 10, () => Math.floor(Math.random() * 150) + 350 * 0.95);
                        console.log(links);
                        if (links.length > 0) {
...
async function getNode(selector, maxAttempts, getDelay, options = {}) {
    let count = 0;
    let node = null;
